...
/**
 * bin_tr_is_heap - checks if a binary tree is a valid Max Binary Heap
...
int bin_tr_is_heap(const binary_tree_t *tree)
{
	if (!tree)
		return (0);
	return (btih_help(tree));
}

/**
 * btih_help - checks if a binary tree is a valid Max Binary Heap
 * @tree: a pointer to the root node of the tree to check
 *
 * Return: 1 if tree is a valid Max Binary Heap
 *         1 if tree is NULL
...
int btih_help(const binary_tree_t *tree)
{
	...
	return (btih_help(tree->left) &&
		btih_help(tree->right));
}
